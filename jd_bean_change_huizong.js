...
const notifyPath = path.resolve(currentDir, './sendNotify');
console.log('notify 对应的文件路径:', notifyPath);
...
!(async () => {
    ...
    for (i = 0; i < cookiesArr.length; i++) {
        if (cookiesArr[i]) {
            ...
            $.UserNameMe = decodeURIComponent(cookie.match(/username=([^; ]+)(?=;?)/) && cookie.match(/username=([^; ]+)(?=;?)/)[1]);
            ...
            console.log(`*********开始查询【账号${$.index}】${$.nickName || $.UserNameMe || $.UserName}***********`);
            ...
            var info = await showMsg();
            all_msg.push(info['msg']);
            if(info['barkDetail']){
                all_bark.push(`【京东账号${$.index}】${$.UserNameMe || $.nickName || $.UserName}\n` + info['barkDetail'])
            }
    ...
    if(all_msg && all_msg.length > 0){
        await notify.sendNotifybyWxPucher('所有账户资产汇总', all_msg.join('\n'), 'a309871271', undefined, undefined);
    }

    if(all_bark && all_bark.length > 0){
        const options = {
            url: `https://bark.6tun.com/dvvFu9p3TvZHrHipusfUKi`,
            json: {
                title: '资产领取通知',
                body: all_bark.join('\n'),
                group: 'qinglong',
...
async function showMsg() {
    ...
    if ($.beanCount) {
        ...
        var money = parseFloat((($.beanCount - $.beanChangeXi) / 100).toFixed(2))
        if(money > 50){
            barkDetail += `【当前京豆】：${$.beanCount - $.beanChangeXi}豆\n`
        }
    ...
    if ($.JdFarmProdName != "") {
        if ($.JdtreeEnergy != 0) {
            if ($.treeState === 2 || $.treeState === 3) {
                ...
                barkDetail += `【老农场】${$.JdFarmProdName} 可以兑换了!\n`;
                if (userIndex2 != -1) {
                ...
                var NCprocess = parseFloat(($.JdtreeEnergy / $.JdtreeTotalEnergy) * 100)
                if (NCprocess > 99){
                    barkDetail += `【老农场】${$.JdFarmProdName} 快成熟了! (${(($.JdtreeEnergy / $.JdtreeTotalEnergy) * 100).toFixed(0)}%) \n`;
                }
                //}
            }
        } else {
    ...
    if ($.fruitnewinfo){
        ...
        if ($.fruitnewinfo.skuName && $.fruitnewinfo.treeFullStage == 5 ){
            ...
            var currentProcess = parseFloat($.fruitnewinfo.currentProcess)
            if (currentProcess > 99){
                barkDetail += `【新农场】快成熟了! ( ${$.fruitnewinfo.currentProcess}%) \n`;
            }
    ...
    if ($.newfarm_info){
            ...
            barkDetail += `【新农场】奖品未兑换: ${$.newfarm_info}\n`;    
            allReceiveMessage += `【新农场】奖品未兑换: ${$.newfarm_info}\n`;        
    }
    ...
    if (dwscore) {
        ...
        if (parseFloat(dwscore) > 12){
            barkDetail += `【话费积分】${dwscore} 可以兑换话费了!\n`;
        }
        ReturnMessage += `\n`;
    }
    ...
    if (marketcard && marketcard.balance != '0.00' ) {
        ...
        if (parseFloat(marketcard.balance) > 12){
            barkDetail += `【超市卡】${marketcard.balance}元` 
            barkDetail += `(${marketcard.expirationGiftAmountDes})`
        }
    ...
    if ($.sqb_info != '' ) {
        ...
        const match = $.sqb_info.match(/(\d+)/); // 使用正则表达式匹配数字

        const sqb = 0
        if (match) {
            const sqb = match[0]; // 提取到的数字
        } else {
            console.log("没有找到数字");
        }

        if (parseFloat(sqb / 1000 ) > 20){
            barkDetail += `【省钱币】${$.sqb_info}`;
        }
    }        
    if ($.jdCash) {
        ReturnMessage += `【其他信息】`;
    ...
    if (parseFloat(parseFloat($.jdRed) + parseFloat($.jdwxRed) + parseFloat($.jdGeneralRed)) > 5) {
        barkDetail += `红包${$.balance}元`;
    }
    ...
    if ($.isNode() && WP_APP_TOKEN_ONE) {
        ...
        if ($.JingXiang||1) {
            if ($.isRealNameAuth)
                if (cookie.includes("app_open"))
                    ReturnMessage = `【账号名称】${$.UserNameMe || $.nickName || $.UserName}(wskey已实名)\n` + ReturnMessage;
                else
                    ReturnMessage = `【账号名称】${$.UserNameMe || $.nickName || $.UserName}(已实名)\n` + ReturnMessage;
            else
                if (cookie.includes("app_open"))
                    ReturnMessage = `【账号名称】${$.UserNameMe || $.nickName || $.UserName}(wskey未实名)\n` + ReturnMessage;
                else
                    ReturnMessage = `【账号名称】${$.UserNameMe || $.nickName || $.UserName}(未实名)\n` + ReturnMessage;

        } else {
            ReturnMessage = `【账号名称】${$.UserNameMe || $.nickName || $.UserName}\n` + ReturnMessage;
        }
        if (TempBaipiao) {
    ...
    return {
        'msg': '*************** \n' + ReturnMessageTitle + ReturnMessageBK + "\n",
        'barkDetail': barkDetail,
    }
