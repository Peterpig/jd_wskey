...
def log_error(message: str) -> None:
    ...
    """
    print(
        f"\r[{time.strftime('%Y-%m-%d %H:%M:%S', time.localtime())}] [ERROR] {message}"
    )
...
def get_browser(path_prefix: str = "") -> webdriver.Chrome:
    ...
    try:
        ...
        driver_path = (
            get_file(path_prefix + DRIVER_PATHS[sys.platform])
            if sys.platform in DRIVER_PATHS
            else None
        )
...
def handle_webdriver_exception(e: WebDriverException) -> None:
    ...
    for key, message in error_messages.items():
        ...
        log_error(
            f"浏览器错误， 请检查你下载并解压好的驱动是否放在drivers目录下，如需帮助请及时反馈; err: {str(e)}"
        )
